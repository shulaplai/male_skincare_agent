...
retriever in-process — see the positive control in Q5: `recall` 100%→40%, **exit 1**.
...
So "綠先 merge" is a personal discipline, not a property of the repo. Also note that every run in
the observed window (11 most recent, `gh run list --limit 10` plus the failed ones) is a `push` to
`main` — the `pull_request:` trigger in `ci.yml:5` appears never to have fired, so CI has never
actually acted as a merge gate at all.
...
path the product actually runs (`tools.py:11,70`) — and it ignores `judge_scores`. Q5 proves this
is not pedantry: reverting the runtime to pure semantic leaves the harness at exit **0** while
still printing `Hybrid（runtime path）recall: 100% · MRR: 1.00`, and even wrecking `search_hybrid`
outright (recall 100% → 20%) still exits **0**.

Finally, an observation that supports "this gate has never been exercised": since the eval job
was introduced in commit `ad05112` ("fi"), it has **passed in all 7 runs in which it exists**
(33646173524, 33646702578, 33697884566, 34419981670, 34459307123, 34678831932, 34823949127).
The `eval` job has never once failed. Runs before `ad05112` — 33642448734, 33601449194,
33455765852, 33395804752 — report only `frontend` + `backend`, so no eval job existed at all.
...
locally: the positive control in Q5 drove `--fake` to a genuine FAIL and got **exit 1**.
...
- `data/runs.jsonl` (the run log, `config.py:59`, written best-effort by `_write_run_log` at
  `service.py:40,64` behind a `run_log_enabled` gate) is *not* created by the eval, which is
  consistent: the run log is written by the API service, not the harness. Confirmed empirically —
  no `runs.jsonl` exists in `backend/data` after any of my runs.
...
`server.log`) kept byte-identical mtimes across the whole audit, including every sabotage run and
every patched-pytest run. **Isolation is real.** This part of row #12 is VERIFIED.
...
(Honesty note on method: my first version of this sabotage returned the *oldest `top_k`* chunks.
With only 4 corpus chunks that kept hybrid recall pinned at 100%, so it proved nothing and I
discarded it; the single-fixed-chunk version above is what produced the 20%. I then ran the test
suite against it rather than assuming the backend job would stay green, which is how I found that
this variant *is* caught.) So the honest boundary is: the backend job catches hybrid **length**
...
**All three jobs green.** Note this path needs no subversion of a metric at all: it is a plain
behaviour break that no job is looking for, and the failure is user-visible on the very feature
docs row #25 sells as done.
...
CI, and there is no branch protection to make it binding. The concrete regression I would name
is the **runtime retrieval revert** (`tools.search_knowledge` back to pure semantic `retrieve()`):
all three jobs stay green, the backend suite is 73/73, and the eval keeps printing
`Hybrid（runtime path）recall: 100% · MRR: 1.00` for code that no longer runs hybrid. If you want
the largest blast radius per keystroke, a frontend behaviour regression is even easier — no job
looks at frontend behaviour at all.

---

## Q6 — is the `expect_tool` gate real, and would it have caught #26?

**The assertion** (`eval/agent_eval.py:30-33,41-43`):
...
**It is real and it fires.** Simulating "#26 on the wire" — a model that proposes no tools, by
wrapping `FakeLLM.structured` to return `SkinAnalysis(tool_calls=[])`:
...
**But it would NOT have caught #26 in CI.** Replaying the *actual* pre-#26 state (the analyse
prompt never told the model which tools exist, i.e. `TOOL_GUIDE` absent from `ANALYZE_SYSTEM`)
and running the real CI command `python -m eval.run_eval --fake`:
...
does and which needs a key and network. The docstring at `agent_eval.py:7-8` is honest about
...
# Q5 — the regression I would name, plus its boundary (all scratch scripts in /tmp)
./.venv/bin/python /tmp/sabotage_wiring.py           # revert runtime to pure semantic:
                                                     #   eval exit 0, still prints hybrid 100%/1.00
PYTHONPATH=/tmp ./.venv/bin/python -m pytest -q -p revert_wiring_plugin    # 73 passed, exit 0
./.venv/bin/python /tmp/sabotage_eval.py hybrid      # wreck search_hybrid -> eval exit 0 ...
PYTHONPATH=/tmp ./.venv/bin/python -m pytest -q -p hybrid_sabotage_plugin  # ... but pytest exit 1
