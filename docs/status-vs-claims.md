...
| 2 | LangGraph 6-node flow（architecture §2 / README line 6） | 實際 5 nodes：analyze→tools→advise→guardrail→persist；memory read/檢索係 tools（要 LLM 主動 call） | (a) | 🟡 docs drift → 改 architecture/README（demo-script 已寫 5 nodes 啱） |
...
| 4 | Local mode（Ollama）（README/architecture §4） | ❌ 冇 Ollama adapter；`llm_provider=ollama` → FakeLLM。冇 key = FakeLLM（唔係真 offline，embedder 照 load） | (c) | 🟡 降級：docs 改寫「冇 key = FakeLLM 示範模式」；Ollama 唔做（你用雲端 vision） |
| 5 | LangGraph checkpointer 做長期記憶（architecture §3） | ❌ 冇用 checkpointer。實際：stateless consult + SQLite（Entry/memory/messages）+ 最近 10 條 messages context | (a) | 🟡 docs 改寫（Q39 決定：唔用 checkpointer） |
...
| 8 | RAG：PDF→OCR→chunk→embed→sqlite-vec（README） | ✅ RAG 真（chunks + cosine）；但 **冇 OCR**（pypdf 抽 text）；embedding 存 JSON 唔係 sqlite-vec | (a) | 🟡 docs 改字眼；OCR/sqlite-vec 唔需要做（規模細） |
...
| 12 | Eval 入 CI、「綠先 merge」（roadmap P5 / demo-script） | ✅ CI 加 `eval` job（`--fake`，FAIL → exit 1）；eval 行 temp DB + committed golden corpus | (b) | ✅ **真**（Q16）。`docs/eval-report-sample.md` 仍然 stale（寫 2 scenarios / 20 tests）→ 要更新 |
| 13 | 「20 tests 全綠」 | 實際 **40** 個（backend） | (a) | 🟡 更新 eval-report-sample / demo-script 數字 |
...
- **Docs sweep（(a) 類）**：architecture node 數/checkpointer/Ollama 字眼（#2/4/5）、RAG 字眼（#8）、eval-report-sample 同 demo-script 數字（#12/13）、README 部署段（#18）、roadmap Phase 完成度。
